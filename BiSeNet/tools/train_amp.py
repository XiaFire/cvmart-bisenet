...
def set_model():
    ...
    if not args.finetune_from is None:
        ...
        if 'city' in args.finetune_from:
            mismatch = ['head.conv_out.1.weight','head.conv_out.1.bias','aux2.conv_out.1.weight','aux2.conv_out.1.bias','aux3.conv_out.1.weight','aux3.conv_out.1.bias','aux4.conv_out.1.weight','aux4.conv_out.1.bias','aux5_4.conv_out.1.weight', 'aux5_4.conv_out.1.bias']
            for key in mismatch:
                ckpt.pop(key)
        _ = net.load_state_dict(ckpt, strict=False)
        print(_)
...
def train():
    ...
    for it, (im, lb) in enumerate(dl):   
        ...
        if (it + 1) % 5000 == 0:
            # dump the last model
            save_pth = osp.join(cfg.respth, 'model_last.pth')
